...
static void outputUnoptimizedMaps(
	...
	for (AttrmapEntry const &attr : attrmap) {
		...
		if (attr.isBackgroundTile()) {
			...
			// With `-N/--nb-tiles` greater than 256, tile IDs may wrap around in the tilemap, which
...
static void outputTilemap(std::vector<AttrmapEntry> const &attrmap) {
	...
	// With `-N/--nb-tiles` greater than 256, tile IDs may wrap around in the tilemap, which was
...
continue_visiting_tiles:;
	...
	auto checkTileCount = [](size_t nbTiles) {
		...
		} else if (nbTiles > 0xff && !options.tilemap.empty()) {
			// With `-N/--nb-tiles` greater than 256, tile IDs may wrap around in the tilemap, so
			// warn about that.
			warnx(
